...
### PCA Plots for both species *S. maderensis* & *S. aurita*
...
##  PCA Plots for *S. maderensis*:

### *S.maderensis* species
...
# Genetic Analysis
## SNP filtering
a. vcftools --vcf TotalRawSNPs.vcf --max-missing 0.5 --mac 3 --minQ 20 --recode --recode-INFO-all --out raw.g5mac
After filtering, kept 236 out of 236 Individuals
Outputting VCF file...
After filtering, kept 15290 out of a possible 249666 Sites

b.vcftools --vcf raw.g5mac3.recode.vcf --minDP 3 --maf 0.01 --recode --recode-INFO-all --out raw.g5mac3dp3
After filtering, kept 236 out of 236 Individuals
Outputting VCF file...
After filtering, kept 12807 out of a possible 15290 Sites

c. ./filter_missing_ind.sh raw.g5mac3dp3.recode.vcf DP3g95maf05
...
d. ./pop_missing_filter.sh DP3g95maf05.recode.vcf popmap 0.75 1 TRSdp5MIp25
After filtering, kept 204 out of 204 Individuals
Outputting VCF file...
After filtering, kept 1344 out of a possible 12807 Sites

e.  vcfallelicprimitives -k -g TRSdp5MIp25.recode.vcf | sed 's:\.|\.:\.\/\.:g' >       TRSdp5MIp25g9.prim
...
f. vcftools --vcf SNP.DP3g95maf05d.recode.vcf --recode-INFO-all --max-alleles 2 --out SNP.5a --recode
After filtering, kept 204 out of 204 Individuals
Outputting VCF file...
After filtering, kept 1585 out of a possible 1789 Sites

g. convert the vcf file into a bayescan output
...
h.  mawk '!/#/' SNPAM.neutralonly.recode.vcf | wc -l
 1585 Sites FINAL FILTERED SNPS
...
wc(genindB)
           pop         Ind
Total 0.1279823 -0.06910831
pop   0.0000000 -0.22601678
$FST
[1] 0.1279823
...
## S.aurita Analysis
## Filtering codes
A. vcftools --vcf TotalRawSNPs.vcf --max-missing 0.5 --mac 3 --minQ 20 --recode --recode-INFO-all --out raw.g5mac3
After filtering, kept 115 out of 115 Individuals
Outputting VCF file...
After filtering, kept 341983 out of a possible 1256073 Sites

B. ./filter_missing_ind.sh raw.g5mac3.recode.vcf DP3g95maf05
...
The 85% cutoff would be 0.733832
Would you like to set a different cutoff, yes or no
no
...
C. vcftools --vcf DP3g95maf05.recode.vcf --minDP 5 --maf 0.01 --recode --recode-INFO-all --out raw.g5mac3dp3

After filtering, kept 100 out of 100 Individuals
Outputting VCF file...
After filtering, kept 276130 out of a possible 341983 Sites

D. ./pop_missing_filter.sh raw.g5mac3dp3.recode.vcf popmapF 0.9 1 TRSdp5MIp25
After filtering, kept 100 out of 100 Individuals
Outputting VCF file...
After filtering, kept 58019 out of a possible 276130 Sites

E. ./dDocent_filters TRSdp5MIp25.recode.vcf  TRSdp5MIp25g9dd
...
F. vcftools --vcf TRSdp5MIp25g9dd.FIL.recode.vcf --maf 0.05 --recode --recode-INFO-all --out DP3g95maf05d

After filtering, kept 100 out of 100 Individuals
Outputting VCF file...
After filtering, kept 6400 out of a possible 8164 Sites

G. filter_hwe_by_pop.pl -v DP3g95maf05d.recode.vcf -p popmap -c 0.5 -o SNP.DP3gdp5MIp25g9HWE
...
H. rad_haplotyper.pl -v SNP.DP3gdp5MIp25g9HWE.recode.vcf  -x 10 -mp 2 -r reference.fasta
less stats.out
...
I. vcfallelicprimitives -k -g SNP.DP3gdp5MIp25g9HWE.filtered.vcf| sed 's:\.|\.:\.\/\.:g' > TRSdp5MIp25g9.prim
...
J. vcftools --vcf SNP.DP3g95maf05d.recode.vcf  --recode-INFO-all --max-alleles 2 --out SNP.2a --recode

After filtering, kept 100 out of 100 Individuals
Outputting VCF file...
After filtering, kept 2776 out of a possible 2847 Sites

I. (rad_haplotyper_env) [evelyn-takyi@n038 Aurita_Final2]$ vcftools --vcf SNP.2a.recode.vcf --recode-INFO-all --max-missing 0.5 --out SNP.dp5a --recode

After filtering, kept 100 out of 100 Individuals
Outputting VCF file...
After filtering, kept 1393 out of a possible 2776 Sites
Run Time = 1.00 seconds
(rad_haplotyper_env) [evelyn-takyi@n038 Aurita_Final2]$ mawk '!/#/' SNP.dp5a.recode.vcf | wc -l
1393


M. Use PGDSpider to convert SNPs to BayeScan input file.

N. BayeScan2.1_linux64bits SNP1.BS -thin 100 -nbp 30
O. Create a VCF file of SNPs of outliers with a false discovery rate of 10% using VCFtoOutlierOnly.sh


L. ./VCFtoOutlierOnly.sh SNP.dp5a.recode.vcf  SNPDP5_fst.txt  0.1 SNPAUdp  
...
Basic statistics from diveRsity package in R

| Mauritania | Mean Statistics | Senegal  | Mean statistics | Guinea   |         | Ghana    |         | Togo     |         | Benin    |         |
|------------|-----------------|----------|-----------------|----------|---------|----------|---------|----------|---------|----------|---------|
| N          | 14.12           | N        | 13.93           | N        | 2.74    | N        | 12.64   | N        | 15.55   | N        | 10.82   |
| A          | 3882            | A        | 3978            | A        | 3257    | A        | 3987    | A        | 4057    | A        | 3905    |
| %          | 89.21           | %        | 91.31           | %        | 74.87   | %        | 91.56   | %        | 92.98   | %        | 89.7    |
| Ar         | 2.49            | Ar       | 2.6             | Ar       | 1.92    | Ar       | 2.5     | Ar       | 2.59    | Ar       | 2.52    |
| Ho         | 1               | Ho       | 1               | Ho       | 1       | Ho       | 1       | Ho       | 1       | Ho       | 1       |
| He         | 0.56            | He       | 0.57            | He       | 0.54    | He       | 0.57    | He       | 0.57    | He       | 0.57    |
| HWE        | 0               | HWE      | 0               | HWE      | 1       | HWE      | 0       | HWE      | 0       | HWE      | 0       |
| Fis        | -0.7804         | Fis      | -0.75           | Fis      | -0.861  | Fis      | -0.7608 | Fis      | -0.7439 | Fis      | -0.7652 |
| Fis_Low    | -0.7976         | Fis_Low  | -0.7686         | Fis_Low  | -1.2817 | Fis_Low  | -0.7802 | Fis_Low  | -0.7605 | Fis_Low  | -0.7918 |
| Fis_High   | -0.7682         | Fis_High | -0.7384         | Fis_High | -0.328  | Fis_High | -0.7436 | Fis_High | -0.7315 | Fis_High | -0.746  |
|            |                 |          |                 |          |         |          |         |          |         |          |         |
|            |                 |          |                 |          |         
...
| # Pop ID   | Private | Sites | Variant Sites | Polymorphic Sites | % Polymorphic Loci | Obs Het | Exp Het | Nucleotide diversity | Fis     |
|------------|---------|-------|---------------|-------------------|--------------------|---------|---------|----------------------|---------|
| Benin      | 0       | 1383  | 1383          | 1330              | 96.1678            | 0.6296  | 0.3823  | 0.4013               | -0.4602 |
| Ghana      | 0       | 1383  | 1383          | 1333              | 96.3847            | 0.6104  | 0.3744  | 0.3909               | -0.4462 |
| Guinea     | 0       | 1383  | 1383          | 967               | 69.9205            | 0.5388  | 0.308   | 0.4414               | -0.173  |
| Mauritania | 0       | 1383  | 1383          | 1308              | 94.577             | 0.6269  | 0.3759  | 0.3902               | -0.4808 |
| Senegal    | 0       | 1383  | 1383          | 1355              | 97.9754            | 0.6317  | 0.3895  | 0.4045               | -0.467  |
| Togo       | 0       | 1383  | 1383          | 1359              | 98.2646            | 0.6246  | 0.3876  | 0.401                | -0.4631 |
...
wc(genindB)
              ...
              pop        Ind
Total 0.006426312 -0.5944244
pop   0.000000000 -0.6047369
$FST
[1] 0.006426312
...
## Pairwise FST values calculated in genitive
 Values of F_st for all pairs of populations:

Statistic	BA_1E.res	GA_1B.res	GUI_13	MA_1A	SA_1C.res	TA_1Dres
BA_1E.res	--	-0.002	0.054	-0.000	0.004	0.003
GA_1B.res	-0.002	--	0.063	0.001	0.007	0.007
GUI_13	0.054	0.063	--	0.072	0.029	0.029
MA_1A	-0.000	0.001	0.072	--	0.009	0.009
SA_1C.res	0.004	0.007	0.029	0.009	--	-0.002
TA_1Dres	0.003	0.007	0.029	0.009	-0.002	--
...
![Plot3](https://github.com/evelyn-takyi/Sardinella-project/blob/master/Aurita_str.png)
...
```
# S. maderensis filtering codes

A. vcftools --vcf TotalRawSNPs.vcf --max-missing 0.5 --mac 3 --minQ 20 --recode --recode-INFO-all --out raw.g5mac3
After filtering, kept 122 out of 122 Individuals
Outputting VCF file...
...
D. ./pop_missing_filter.sh raw.g5mac3dp3.recode.vcf popmap 0.9 1 TRSdp5MIp25
After filtering, kept 106 out of 106 Individuals
Outputting VCF file...
...
G. rad_haplotyper.pl -v SNP.DP3gdp5MIp25g9HWE.recode.vcf  -x 10 -mp 2 -r reference.fasta
(rad_haplotyper_env) [evelyn-takyi@n038 Aurita_Final2]$ cp stats.out stats.out.HF
(rad_haplotyper_env) [evelyn-takyi@n038 Aurita_Final2]$ mawk '/Missi/' stats.out.HF | mawk '$9 > 30' > HF.missing
(rad_haplotyper_env) [evelyn-takyi@n038 Aurita_Final2]$ mawk '/para/' stats.out.HF > HF.para
(rad_haplotyper_env) [evelyn-takyi@n038 Aurita_Final2]$ ./remove.bad.hap.loci.sh HF.loci.tofilter  SNP.DP3gdp5MIp25g9HWE.recode.vcf  
...
H. vcfallelicprimitives -k -g SNP.DP3gdp5MIp25g9HWE.filtered.vcf| sed 's:\.|\.:\.\/\.:g' > TRSdp5MIp25g9.prim
 vcftools --vcf TRSdp5MIp25g9.prim --recode-INFO-all --recode --out SNP.DP3g95maf05d --remove-indels
...
I. (rad_haplotyper_env) [evelyn-takyi@n039 No_NM]$ vcftools --vcf SNP.DP3g95maf05d.recode.vcf  --recode-INFO-all --max-alleles 2 --out SNP.2a --recode

After filtering, kept 106 out of 106 Individuals
Outputting VCF file...
After filtering, kept 1562 out of a possible 1592 Sites

J. Use PGDSpider to convert SNPs to BayeScan input file.
...1. BayeScan2.1_linux64bits SNP1.BS -thin 100 -nbp 30
...2. Create a VCF file of SNPs of outliers with a false discovery rate of 10% using VCFtoOutlierOnly.sh

K. ./VCFtoOutlierOnly.sh SNPM.2a.recode.vcf  SNPMA_fst.txt  0.1 SNPMAdp  
...
```
pop         Ind
Total 0.2473595  0.05125541
pop   0.0000000 -0.26055485
$FST
[1] 0.2473595
...
### Pairwise FST values calculated in genodive
 ...
 Statistic	  Benin 	Guinea	Mauritania	Nigeria 	Senegal 	Togo
 Benin      	--	   -0.003	  -0.000	  0.392	     0.002	 -0.005
 Guinea	      -0.003	 --	    -0.007	  0.339	     0.006   	0.016
 Mauritania	  -0.000	-0.007	 --	      0.381	     -0.002	  0.007
 Nigeria      0.392	   0.339	 0.381	   --       	0.368	  0.374
 Senegal 	    0.002	   0.006	 -0.002	   0.368      	--	  0.009
 ...
 pop_tree <- aboot(bov_pop, sample = 1000,
                            cutoff = 50)
![plot1](https://github.com/evelyn-takyi/Sardinella-project/blob/master/Tree_MAD.png)
